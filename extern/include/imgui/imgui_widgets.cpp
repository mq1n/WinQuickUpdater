...
void ImGui::Columns(int columns_count, const char* id, bool border, bool allow_resize)
{
    ImGuiWindow* window = GetCurrentWindow();
    IM_ASSERT(columns_count >= 1);
    ...
    if (border)
        flags |= ImGuiColumnsFlags_NoBorder;
    if (!allow_resize)
        flags |= ImGuiColumnsFlags_NoResize;
    // flags |= ImGuiColumnsFlags_NoPreserveWidths;
