...
//          m_pkLoggerImpl->set_error_handler(LogErrorHandler);
	...
	void CLog::Log(int32_t nLevel, const std::string& stBuffer)
	{
		try
		{
			switch (nLevel)
	...
	void CLog::LogError(const std::string& stMessage, bool bFatal, bool bShowLastError)
	{
		const auto err = CError{ stMessage, ::GetLastError() };
